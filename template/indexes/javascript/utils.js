...
var GeneralContainerName    = "container";
...
function initializePage() {
	...
	for(var unitIndex in DescartesUnits) {
		...
		for(var sceneIndex in currentUnit.Files) {
		   // primero créditos e información para no alterar el tamaño del arreglo
		   // de escenas DescartesScenes.
		   CreditsFiles[DescartesScenes.length] = currentUnit.Credits[sceneIndex];
			InfoFiles[DescartesScenes.length] = currentUnit.Info[sceneIndex];
...
function SetContainerSize () {
    ...
    ContentFrameWidth = ContentFrameWidth == -1 ? 790 : ContentFrameWidth < 790 ? 790 : ContentFrameWidth;

    var availableArrowSpace = ShowNavigationArrows ? 0 : 90;
	var availableCloseSpace = SystemCloseButton ? 0 : 30;
    NavigationPanelWidth = ContentFrameWidth + availableArrowSpace + availableCloseSpace - 195;

	$("#" + ContentFrameName).css("width", ContentFrameWidth + "px");
	$("#" + GeneralContainerName).css("width", ContentFrameWidth + "px");
	$("#" + NavigationPanelName).css("width", NavigationPanelWidth + "px");


	ContentFrameHeight = ContentFrameHeight == -1 ? 520 : ContentFrameHeight < 520 ? 520 : ContentFrameHeight;
	var generalContainerHeight = ContentFrameHeight + 54;
	$("#" + ContentFrameName).css("height", ContentFrameHeight + "px");
	$("#" + GeneralContainerName).css("height", generalContainerHeight + "px");
...
function calculateButtonWidth () {
	...
	buttonWidth = (NavigationPanelWidth - (unitsLength * 5)) / unitsLength;
...
function verDocumentacion() {
	...
	window.open (InfoFiles[currentSceneIndex], 'documentacion', cfgWin);
}

function verCreditos() {
	var cfgWin = 	"width=410, height=400, resizable=no, location=no, menubar=no, ";
	cfgWin += 		"status=no, titlebar=no, toolbar=no, scrollbars=1";
	window.open (CreditsFiles[currentSceneIndex], 'creditos', cfgWin);
}
